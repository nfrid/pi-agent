...
test('does not borrow a following visit when restoring a manually read thread', async ({
  page,
}) => {
  const sessions = [
    { ...snapshot.sessions[0], title: 'Live thread A' },
    { ...snapshot.sessions[1], title: 'Remembered thread B' },
  ];
  const sessionEntries = (id: string, liveUpdate = false) => [
    ...Array.from({ length: 120 }, (_, index) => ({
      type: 'message',
      id: `${id}-${index}`,
      message: {
      ...
      ? [
          {
            type: 'message',
            id: `${id}-live-update`,
            message: { role: 'assistant', content: `${id} live update` },
          },
        ]
      : []),
  ];
  ...
  await page.route('**/trpc/sessionSubscribe*', async (route) => {
    ...
    const id = input.sessionId ?? 'session-1';
    const count = (subscribeCounts.get(id) ?? 0) + 1;
    subscribeCounts.set(id, count);
    const index = id === 'session-2' ? 1 : 0;
    return route.fulfill({
      contentType: 'text/event-stream',
      body: trpcSseData(
        {
          ...
          snapshot: {
            ...
            entries: sessionEntries(id, id === 'session-2'),
            entriesComplete: false,
            serverId: snapshot.serverId,
            cursor: count,
            active: {
  ...
  const savedAnchor = await transcriptScroll(page).evaluate((element) => {
    ...
    return {
      key: row?.dataset.transcriptKey,
      offset: row ? row.getBoundingClientRect().top - viewportTop : Number.NaN,
    };
  });
  expect(savedAnchor.key).toBeTruthy();
  await navigateInDashboard(page, '/sessions/session-1');
  ...
  await expect
    ...
    .toBeGreaterThanOrEqual(savedAnchor.offset - 8);
  await expect
    .poll(() =>
      restoredRow.evaluate(
        (row) =>
    ...
    .toBeLessThanOrEqual(savedAnchor.offset + 8);
});
