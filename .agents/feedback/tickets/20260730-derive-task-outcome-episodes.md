...
- **Merged change:** `76c6261` (implementation `e1acaf6 feat(session-metrics): derive task outcome episodes`); review fixes `204a258` (implementation `dac27cb fix(session-metrics): harden episode regressions`) and `5413ef3` (implementation `70b5feb fix(episodes): tighten disposition and recovery inference`)
