...
3. Verify the web entrypoint returns `200`, the authenticated protocol-v2 browser queries load, and the removed finite/session endpoints remain gone:
   `curl -fsS -o /dev/null "http://127.0.0.1:${PI_DASHBOARD_WEB_PORT:-4174}/" && curl -fsS -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/trpc/protocolInfo" >/dev/null && curl -fsS -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/trpc/shellSnapshot?input=%7B%22protocolVersion%22%3A2%7D" >/dev/null && test "$(curl -s -o /dev/null -w '%{http_code}' -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/trpc/bootstrap?input=%7B%22protocolVersion%22%3A1%7D")" = 404 && test "$(curl -s -o /dev/null -w '%{http_code}' -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/api/sessions/example")" = 404`
