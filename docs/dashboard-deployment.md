...
3. Verify the web entrypoint returns `200`, the authenticated protocol-v2 shell snapshot loads, old finite/session endpoints are gone, and the unchanged event/delegate routes remain available:
   `curl -fsS -o /dev/null "http://127.0.0.1:${PI_DASHBOARD_WEB_PORT:-4174}/" && curl -fsS -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/trpc/protocolInfo" >/dev/null && curl -fsS -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/trpc/shellSnapshot?input=%7B%22protocolVersion%22%3A2%7D" >/dev/null && test "$(curl -s -o /dev/null -w '%{http_code}' -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/trpc/bootstrap?input=%7B%22protocolVersion%22%3A1%7D")" = 404 && test "$(curl -s -o /dev/null -w '%{http_code}' -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/api/sessions/example")" = 404 && curl -fsS -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/api/events" >/dev/null && curl -fsS -H "x-dashboard-token: $PI_DASHBOARD_AUTH_TOKEN" "http://127.0.0.1:${PI_DASHBOARD_PORT:-4173}/api/sessions/example/delegate-history" >/dev/null`
