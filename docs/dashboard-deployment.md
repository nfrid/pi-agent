...
3. The runner loads `.env.dashboard` from the repository root before applying LaunchAgent environment overrides; keep the dashboard token and other secrets in that file, not in the plist. Source `.env.dashboard` without printing its token, then wait for both the bridge socket and HTTP health endpoint:
