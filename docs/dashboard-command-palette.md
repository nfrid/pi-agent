...
- Thread results use two compact lines: title with lifecycle/live status and relative activity, then `project / branch` with creation date. Exact activity/creation timestamps and checkout kind/path remain available through tooltips. Project, branch, checkout kind/path, live status, and lifecycle are searchable.
