    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          const contextInfo =
            ...
            contextUsage.tokens !== null
              ? theme.fg(
                  ...
                  `${contextPercent}% [${formatTokenCount(contextUsage.tokens)}/${formatTokenCount(contextWindow)}]`,
                )
              : '';

          const lastAssistant = getLastAssistantMessage(ctx);
          const cacheRead = lastAssistant?.usage.cacheRead ?? 0;
          const cacheInfo =
            lastAssistant && cacheRead > 0
              ? theme.fg('success', `cache ${formatTokenCount(cacheRead)}`)
              : '';
          ...
          const right = [cacheInfo, contextInfo];
          const rightText = joinParts(theme, width - leadingSpaceWidth, right);
          ...
          const leftWidth = Math.max(
            0,
            width - leadingSpaceWidth - minGapWidth - rightWidth,
          );
          const leftText =
            leftWidth > 0 ? joinParts(theme, leftWidth, left) : '';
          const gapWidth = Math.max(
            0,
            width - leadingSpaceWidth - visibleWidth(leftText) - rightWidth,
          );
