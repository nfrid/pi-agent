...
describe('writable delegate isolation', () => {
  ...
  test.skipIf(!sandboxBackendAvailable())(
    'terminates an actual isolated child before another model turn exceeds local budgets',
    async () => {
      const result = await prepareWritableIsolation({
        cwd: repository,
        scopes: ['src'],
      });
      const sessionPath = path.join(agentDir, 'child-budget.jsonl');
      writeFileSync(sessionPath, '{}\n');
      ...
      const event = JSON.stringify({
        ...
        message: {
          role: 'assistant',
          content: [{ type: 'text', text: 'tool needed' }],
          usage: {
            input: 10,
            output: 2,
            cacheRead: 0,
      ...
      try {
        ...
        expect(run.errorMessage).toMatch(/turn budget 1 reached/);
        expect(run.usage).toMatchObject({ turns: 1, computeUnits: 3 });
        const captured = await captureIsolationPatch(prepared.record.id, {
          outcome: 'error',
        });
        await discardIsolation(captured.id);
