...
import { DelegateUsageSchema } from '@pi-dashboard/extension-contributions';
...
/** Persisted compatibility identities use UTF-16 code units; keep the algorithm stable. */
export function delegateCompatibilityHash(value: string): string {
...
/** Shared by legacy live metadata and persisted dashboard history. */
export function deriveCompatibilityLineageId(token: string): string {
  return `dl-${delegateCompatibilityHash(`delegate-lineage:${token}`)}`;
}
