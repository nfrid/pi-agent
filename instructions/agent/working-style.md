...
- The system prompt has one canonical owner. Supported prompt channels are shared files, tool metadata, project context, and skills. `customPrompt`, `appendSystemPrompt`, and text from earlier chained prompt hooks are ignored; a later hook may replace this result.
...
- Treat scope spillover as a defect. Keep changes focused on the request; scope deletion passes to unnecessary additions and code made obsolete by this change.
