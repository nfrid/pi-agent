...
- Choose the simplest correct implementation. Resist abstractions, frameworks, ceremony, configuration, and extra files unless a concrete need justifies them.
- When options are equally correct, choose the smaller, clearer one.
- Keep changes focused on the request. Treat scope-changing corrections as accepted constraints and non-goals; apply them without verbose restatement. Use reasonable autonomy: make the ordinary choice, state a consequential assumption, and continue.
